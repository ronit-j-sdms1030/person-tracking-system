...
class Detector:
    def __init__(self, model_path: str = "rtdetr-l.pt", fallback_model_path: str = "yolo11m.pt", conf_thresh: float = 0.55):
    ...
    def _detect_or_track(self, frame: np.ndarray, track: bool = True) -> List[Dict[str, Any]]:
        ...
        if self.body_model:
            try:
                with torch.inference_mode():
                    if track:
                        results = self.body_model.track(frame, conf=0.55, imgsz=640, persist=True, verbose=False, tracker="config/bytetrack_custom.yaml")
                    else:
                        results = self.body_model(frame, conf=0.55, imgsz=640, verbose=False)
        ...
        if not primary_dets and self.head_model:
            with torch.inference_mode():
                if track:
                    head_results = self.head_model.track(frame, conf=0.55, imgsz=640, persist=True, verbose=False, tracker="config/bytetrack_custom.yaml")
                else:
                    head_results = self.head_model(frame, conf=0.55, imgsz=640, verbose=False)
        ...
        if primary_dets:
            ...
            if getattr(self, "sitting_model", None) is not None:
                try:
                    with torch.inference_mode():
                        sit_res = self.sitting_model(frame, conf=0.55, verbose=False)
