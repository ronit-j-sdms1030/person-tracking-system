...
let wizardActive = true; // true = wizard is showing, block renderZone from rebuilding camera UI
...
function renderZone(zoneData) {
 ...
 // Block all rendering while the setup wizard is active (page load / hard refresh)
 ...
 zones = [
 { name: " Main Lounge Sofa", prefix: "Sofa", count: 3 },
...
function buildCameraCards(cameraList) {
 ...
 <button onclick="deleteCamera('${camId}')" style="background:none; border:none; color:var(--red); cursor:pointer; font-size:16px;" title="Delete Feed"></button>
...
async function uploadCameras() {
 ...
 alert(` Video Upload Notice: ${error.message || error}`);
 } finally {
 if (uploadBtn) {
...
function openCameraSetupWizard() {
 wizardActive = true; // Lock: block WebSocket renderZone from overriding wizard
 const wizard = document.getElementById('initial-setup-wizard');
 if (wizard) wizard.style.display = 'block';

 const slotContainer = document.getElementById('wizard-slot-container');
...
function configureWizardSlots(count) {
 ...
 html += `
 <div style="background:var(--panel); border:1px solid var(--panel-border); border-radius:10px; padding:14px; display:flex; flex-direction:column; gap:10px;">
 ...
 <span style="font-family:'JetBrains Mono',monospace; font-size:11px; color:var(--muted);">(${slotName})</span>
 </div>
 <div style="display:flex; gap:0; background:var(--panel-2); border:1px solid var(--panel-border); border-radius:8px; overflow:hidden;">
 <button onclick="switchCamInputMode(${i},'rtsp')" id="tab-rtsp-${i}" style="font-family:'JetBrains Mono',monospace; font-size:11px; font-weight:700; padding:5px 14px; border:none; background:var(--orange); color:#fff; cursor:pointer;">Live RTSP</button>
 <button onclick="switchCamInputMode(${i},'file')" id="tab-file-${i}" style="font-family:'JetBrains Mono',monospace; font-size:11px; font-weight:700; padding:5px 14px; border:none; background:transparent; color:var(--muted); cursor:pointer;">Video File</button>
 </div>
 </div>
 <div id="input-rtsp-${i}" style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
 <input type="text" id="wizard-rtsp-${i}" placeholder="rtsp://user:pass@ip:554/Streaming/Channels/101"
 style="flex:1; background:var(--panel-2); color:var(--text); border:1px solid var(--orange); padding:8px 12px; border-radius:8px; font-family:'JetBrains Mono',monospace; font-size:12px; outline:none; min-width:260px;">
 </div>
 <div id="input-file-${i}" style="display:none; align-items:center; gap:8px; flex-wrap:wrap; margin-bottom:8px;">
 ...
 <span style="font-size:11.5px; color:var(--muted); font-family:'JetBrains Mono',monospace; font-weight:600;">Model:</span>
 <select id="wizard-model-${i}" style="background:var(--panel); color:var(--orange); border:1px solid var(--panel-border); padding:3px 6px; border-radius:6px; font-family:'JetBrains Mono',monospace; font-size:11.5px; outline:none; font-weight:700; cursor:pointer;">
 <option value="rtdetr" selected>RT-DETR (Default)</option>
 <option value="yolo">Fine-Tuned YOLOv8 (best.pt)</option>
 </select>
 </div>
 <div style="display:flex; align-items:center; gap:6px; background:var(--panel-2); border:1px solid var(--panel-border); padding:4px 8px; border-radius:8px;">
...
function switchCamInputMode(i, mode) {
 const rtspDiv = document.getElementById(`input-rtsp-${i}`);
 const fileDiv = document.getElementById(`input-file-${i}`);
 const tabRtsp = document.getElementById(`tab-rtsp-${i}`);
 const tabFile = document.getElementById(`tab-file-${i}`);
 if (mode === 'rtsp') {
 rtspDiv.style.display = 'flex';
 fileDiv.style.display = 'none';
 tabRtsp.style.background = 'var(--orange)'; tabRtsp.style.color = '#fff';
 tabFile.style.background = 'transparent'; tabFile.style.color = 'var(--muted)';
 } else {
 rtspDiv.style.display = 'none';
 fileDiv.style.display = 'flex';
 tabFile.style.background = 'var(--orange)'; tabFile.style.color = '#fff';
 tabRtsp.style.background = 'transparent'; tabRtsp.style.color = 'var(--muted)';
 }
}
...
async function submitWizardCameras() {
 ...
 const modelInput = document.getElementById(`wizard-model-${i}`);
 const cap = parseInt(capInput ? capInput.value : 25) || 25;
 const selModel = modelInput ? modelInput.value : 'rtdetr';

 // Check if RTSP tab is active (rtsp div visible)
 const rtspDiv = document.getElementById(`input-rtsp-${i}`);
 const rtspInput = document.getElementById(`wizard-rtsp-${i}`);
 const isRtspMode = rtspDiv && rtspDiv.style.display !== 'none';

 if (isRtspMode && rtspInput && rtspInput.value.trim().startsWith('rtsp')) {
 rtspCameras.push({ slot: slotName, url: rtspInput.value.trim(), capacity: cap, selected_model: selModel });
 fileAdded = true;
 } else {
 ...
 formData.append('cam_capacities', cap);
 formData.append('selected_models', selModel);
 fileAdded = true;
 }
 }
 }
 ...
 if (capInput && capInput.value) formData.append('capacity', parseInt(capInput.value));

 if (!fileAdded) {
 alert("Please enter an RTSP URL or choose a video file for each camera slot.");
 return;
 }

 const launchBtn = document.getElementById('wizard-launch-btn');
 if (launchBtn) { launchBtn.disabled = true; launchBtn.textContent = " Setting Up Cameras..."; }

 try {
 const addedCams = [];

 // Handle RTSP cameras via dedicated endpoint
 for (const cam of rtspCameras) {
 const res = await fetch('/add-rtsp-camera', {
 method: 'POST',
 headers: { 'Content-Type': 'application/json' },
 body: JSON.stringify({ camera_id: cam.slot, source: cam.url, role: 'both', capacity: cam.capacity, selected_model: cam.selected_model })
 });
 if (res.ok) addedCams.push(cam.slot);
 }

 // Handle file cameras via existing endpoint
 if ([...formData.keys()].includes('files')) {
 const res = await fetch('/upload-cameras', { method: 'POST', body: formData });
 if (res.ok) {
 const result = await res.json();
 if (result.cameras_added) addedCams.push(...result.cameras_added);
 } else {
 const errText = await res.text();
 alert(`Failed to upload cameras: ${res.status} - ${errText}`);
 }
 }

 if (addedCams.length > 0) {
 buildCameraCards(addedCams);
 selectCam('both', document.querySelectorAll('.cam-select button')[wizardSelectedCount]);
 }
 } catch(e) {
 alert(`Error: ${e}`);
 } finally {
 if (launchBtn) { launchBtn.disabled = false; launchBtn.textContent = " Confirm & Launch Cameras"; }
 }
 
}

function revealDashboardPanels(cameraCount) {
 wizardActive = false; // Unlock: WebSocket can now update dashboard UI
 const wizard = document.getElementById('initial-setup-wizard');
