...
if ret:
    ...
    sitting = sum(1 for d in detections if d.get('posture_state') == 'sitting')
    standing = sum(1 for d in detections if d.get('posture_state') == 'standing')
    unknown = sum(1 for d in detections if d.get('posture_state') not in ('sitting', 'standing'))
